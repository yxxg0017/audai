...
2. 启动项目，说明 `.env.local` 中需要配置 OpenAI API Key。
3. 点击“开始”，授权摄像头和麦克风。
4. 展示摄像头预览、麦克风电平和本地媒体状态。
5. 点击“分析画面”，展示抽帧预览和视觉分析结果。
6. 点击“连接语音”，展示 Realtime 连接状态。
7. 语音提问“我面前有什么”，展示视觉上下文注入和 AI 回答。
8. 在 AI 回复时继续说话，展示插话打断。
9. 展示成本控制面板：视觉请求次数、缓存命中、图片体积和缓存窗口。
10. 讲解成本控制策略：不传连续视频、低细节图片、60 秒缓存、API 限流和短回答。
