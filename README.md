...
首次进入页面会先显示配置页。你可以输入 OpenAI API Key、Base URL 和模型名称，配置会保存到当前浏览器的 localStorage。下次打开同一浏览器时会自动进入主界面，无需重复输入。
...
视觉分析默认使用低细节图片输入和较短输出，以控制调用成本。Realtime 语音会话通过服务端创建临时 client secret，浏览器只使用短期凭据建立 WebRTC 连接。实时语音默认开启服务端 VAD、中文转写和用户插话打断，减少前端手写音频分段逻辑。
...
3. 保存 API Key、Base URL 和模型配置，说明下次进入无需重复输入。
4. 点击“开始”，授权摄像头和麦克风。
5. 展示摄像头预览、麦克风电平和本地媒体状态。
6. 点击“分析画面”，展示抽帧预览和视觉分析结果。
7. 点击“连接语音”，展示 Realtime 连接状态。
8. 语音提问“我面前有什么”，展示视觉上下文注入和 AI 回答。
9. 在 AI 回复时继续说话，展示插话打断。
10. 通过顶部“设置”菜单展示配置管理。
11. 展示成本控制面板：视觉请求次数、缓存命中、图片体积和缓存窗口。
